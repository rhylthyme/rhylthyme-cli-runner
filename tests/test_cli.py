...
class TestCLIValidation:
    ...
    def test_validate_with_environment(self, cli_runner, kitchen_program_file, kitchen_environment_file):
        """Test validating a program with environment."""
        from rhylthyme_cli_runner.cli import cli
        result = cli_runner.invoke(cli, [
            'validate', 
            kitchen_program_file, 
            '-e', kitchen_environment_file
        ])
...
class TestCLIErrorHandling:
    ...
    def test_validate_with_invalid_environment_file(self, cli_runner, simple_program_file, temp_dir):
...
def test_cli_with_real_examples(cli_runner, examples_dir):
    ...
    if json_files:
        ...
        for filepath in json_files[:3]:
            ...
            if result.exit_code != 0:
                print(f"⚠️ Example {os.path.basename(filepath)} failed validation: {result.output}")
    else:
