...
try:
    from .validate_program import load_program_file, validate_program, perform_additional_validations
    ...
    def validate_program(program: Dict[str, Any], schema: Dict[str, Any]) -> Tuple[bool, List[str]]:
        return True, []
...
class Step:
    ...
    def __init__(self, step_data, track_id, batch_index=0):
        ...
        if "duration" in step_data:
            ...
            if isinstance(duration, (int, float)):
                ...
                if duration_type == "fixed":
                    ...
                    self.default_seconds = float(duration.get("defaultSeconds", (self.min_seconds + self.max_seconds) / 2 if self.max_seconds < float("inf") else self.min_seconds + 60))
                    # Debug logging for cook-bacon step
                    if self.step_id == "cook-bacon":
                        print(f"DEBUG: cook-bacon variable duration: type={self.duration_type}, min={self.min_seconds}, max={self.max_seconds}, default={self.default_seconds}")
                        print(f"DEBUG: cook-bacon triggerName: {duration.get('triggerName')}")
                    ...
                    self.min_seconds = float(duration.get("minSeconds", 0))
                    self.default_seconds = float(duration.get("defaultSeconds", self.min_seconds + 60))
                    self.manual_trigger_name = duration.get("triggerName")
        ...
        if "logic" in start_trigger_data and "triggers" in start_trigger_data:
            ...
            self.start_triggers = start_trigger_data["triggers"]  # List of trigger objects
    ...
    def to_dict(self):
        return {
            ...
            "expectedEndTime": self.expected_end_time.isoformat() if self.expected_end_time else None,
    ...
    def is_ready_to_start(self, completed_steps: Set[str], program_start_time: float, current_time: float, aborted_steps: Set[str] = None) -> bool:
        """Check if the step is ready to start based on its start trigger(s)."""
        if aborted_steps is None:
            aborted_steps = set()
            
        if self.status != StepStatus.PENDING and self.status != StepStatus.WAITING_FOR_MANUAL:
            return False
        ...
        if self.start_triggers is not None and self.start_trigger_logic is not None:
            ...
            for trigger in self.start_triggers:
                trigger_ready = self._evaluate_single_trigger(trigger, completed_steps, program_start_time, current_time, aborted_steps)
        ...
        # Handle single trigger (backward compatibility)
            ...
            return self._evaluate_single_trigger(self.start_trigger, completed_steps, program_start_time, current_time, aborted_steps)
        
        return False
    
    def _evaluate_single_trigger(self, trigger: Dict[str, Any], completed_steps: Set[str], program_start_time: float, current_time: float, aborted_steps: Set[str]) -> bool:
    ...
    def has_manual_trigger(self) -> bool:
        ...
        if self.start_triggers is not None:
            return any(trigger.get("type") == "manual" for trigger in self.start_triggers)
...
class ProgramRunner:
    ...
    def __init__(self, program: Dict[str, Any], time_scale: float = 1.0, auto_start: bool = False):
        """
        ...
        for constraint in resource_constraints:
            ...
            actors_required = constraint.get('actorsRequired', 1.0)  # Default to 1 actor if not specified
            qualified_types = constraint.get('qualifiedActorTypes', [])  # Actor types that can perform this task
            if task:
        ...
        self.manually_triggered_steps = set()  # Initialize manually_triggered_steps as an empty set
                ...
                for step_data in track.get("steps", []):
                    ...
                    if batch_size > 1 and batch_index > 0 and stagger_seconds > 0:
                        ...
                        if trigger_type == "programStart":
                            ...
                            step_data_copy["startTrigger"]["offsetSeconds"] = current_offset + (stagger_seconds * batch_index)
                    ...
                    if start_trigger.get("type") == "afterStep" and batch_size > 1:
                        ...
                        if any(s.get("stepId") == ref_step_id for s in track.get("steps", [])):
                    ...
                    if step.manual_trigger_name:
                        if batch_size > 1:
                            ...
                            batch_trigger_name = f"{step.manual_trigger_name}{batch_suffix}"
    ...
    def start_ready_steps(self, current_time: float) -> None:
        ...
        pending_steps = [step for step in self.steps.values() if step.status == StepStatus.PENDING]
        ...
        for step in pending_steps:
            if self.is_step_ready_to_start(step, current_time):
                ...
                can_start = True
                required_actors_by_type = {}  # Track how many actors of each type are needed
                ...
                for task_type in step.task_types:
                    if task_type in self.resource_constraints:
                        ...
                        for actor_type in qualified_types:
                            ...
                            available_capacity = total_capacity - current_usage - pending_usage
                            
                            if available_capacity >= actors_required and available_capacity > best_available_capacity:
                ...
                if can_start:
                    ...
                    for task_type in step.task_types:
                        if task_type in self.resource_constraints:
                            fraction = step.task_fractions.get(task_type, 1.0)
                            self.resource_usage[task_type] = self.resource_usage.get(task_type, 0.0) + fraction
                    
                    # Update actor usage by type
                    total_actor_usage = 0.0
                    for actor_type, usage in required_actors_by_type.items():
                        self.actor_usage_by_type[actor_type] = self.actor_usage_by_type.get(actor_type, 0.0) + usage
                    ...
                    if step.duration_type == DurationType.FIXED and step.duration_seconds is not None:
                        step.expected_end_time = current_time + step.duration_seconds
                    elif step.duration_type == DurationType.VARIABLE and step.default_seconds is not None:
                        step.expected_end_time = current_time + step.default_seconds
                    elif step.duration_type == DurationType.INDEFINITE and step.default_seconds is not None:
                        step.expected_end_time = current_time + step.default_seconds
                    ...
                    # Emit event
                    self.emit_event("step_started", {"step_id": step.step_id, "time": current_time})
    ...
    def complete_step(self, step: Step, current_time: float) -> None:
        ...
        # Emit event
        self.emit_event("step_completed", {"step_id": step.step_id, "time": current_time})
    
    def trigger_manual_step(self, trigger_name: str, step_id: Optional[str] = None) -> None:
        """
        ...
        if step_id:
            for step in self.manual_triggers[trigger_name]:
                if step.step_id == step_id:
                    ...
                    return
            self.status_message = f"Step with ID {step_id} not found for trigger {trigger_name}"
            return
    ...
    def abort_step(self, step: Step, current_time: float, reason: str = "Aborted") -> None:
        """
        Abort a step.
        ...
        # Emit event
        self.emit_event("step_aborted", {"step_id": step.step_id, "time": current_time, "reason": reason})
    ...
    def get_available_triggers(self) -> List[Dict[str, Any]]:
        ...
        if not self.is_running and self.program.get("startTrigger", {}).get("type") == "manual":
        ...
        for trigger_name, steps in self.manual_triggers.items():
            for step in steps:
                ...
                elif step.status == StepStatus.RUNNING and (step.duration_type == "variable" or step.duration_type == "indefinite"):
                    if step.duration_type == "variable" and step.get_progress(self.current_time) < (step.min_seconds / step.default_seconds) * 100:
    ...
    def get_step_display_info(self, step: Step) -> Dict[str, Any]:
        ...
        return {
            ...
            "progress": progress,
            "remaining": self.format_time(remaining) if remaining is not None else "N/A",
            "task_type": step.task_types[0] if step.task_types else "N/A",  # Keep for backward compatibility
    ...
    def select_next_step(self) -> None:
        ...
        if sorted_steps:
            self.selected_step_index = (self.selected_step_index + 1) % len(sorted_steps)
            self.status_message = f"Selected step: {sorted_steps[self.selected_step_index]['name']}"
                ...
                for constraint in resource_constraints:
                    ...
                    if name:
                        ...
                        result.append({
                            ...
                            "percentage": (current_usage / max_concurrent) * 100 if max_concurrent > 0 else 0
                        })
            else:
    ...
    def get_actor_types_display(self) -> List[Dict[str, Any]]:
        ...
        for actor_type_id, actor_info in self.actor_types.items():
            ...
            percentage = (current_usage / total_capacity * 100) if total_capacity > 0 else 0
    ...
    def display_step(self, step: Step, is_selected: bool = False) -> None:
        ...
        status_display = f"{color}[{self.get_status_display(step.status)}]{Style.RESET_ALL}"
        ...
        if info.get('has_code', False):
            ...
            if info.get('code_executed', False):
                if info.get('code_error'):
                    ...
                    code_display = f" {Fore.GREEN}[{code_type} executed]{Style.RESET_ALL}"
            else:
                code_display = f" {Fore.BLUE}[{code_type} pending]{Style.RESET_ALL}"
        
        # Print the formatted step information
        print(f"{step_display} {status_display} {progress_display} {time_display}{code_display}")
    ...
    def display_help(self) -> None:
        ...
        print("\nCode Execution:")
        print("  Steps can include Python or shell code blocks that execute when the step starts")
    ...
    def get_upcoming_events(self, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        for step in self.steps.values():
            if step.status == StepStatus.PENDING:
                ...
                if estimated_start_time is not None and estimated_start_time >= self.current_time:
                    events.append({
                        "event_type": "start",
                        "step_id": step.step_id,
                        "name": step.name,
                        "time": estimated_start_time,
                        "time_str": self.format_time(estimated_start_time - self.current_time)
                    })
        ...
        for step in self.steps.values():
            if step.status == StepStatus.RUNNING and step.expected_end_time is not None:
                events.append({
                    ...
                    "time_str": self.format_time(step.expected_end_time - self.current_time)
                })
    ...
    def _estimate_step_start_time(self, step: Step) -> Optional[float]:
        ...
        if trigger_type == "programStart":
            ...
            if ref_step.status == StepStatus.COMPLETED and ref_step.end_time is not None:
                base_time = ref_step.end_time if event == "end" else ref_step.start_time
            # If reference step is running and has expected end time
            elif ref_step.status == StepStatus.RUNNING and ref_step.expected_end_time is not None:
                if event == "end":
            ...
            # If reference step is still pending, recursively estimate its start/end time
                ...
                if event == "start":
                    ...
                    if ref_step.duration_type == DurationType.FIXED and ref_step.duration_seconds is not None:
                        base_time = ref_start_time + ref_step.duration_seconds
                    elif ref_step.duration_type == DurationType.VARIABLE and ref_step.default_seconds is not None:
                        base_time = ref_start_time + ref_step.default_seconds
                    elif ref_step.duration_type == DurationType.INDEFINITE and ref_step.default_seconds is not None:
                        base_time = ref_start_time + ref_step.default_seconds
                    else:
                        return None
            else:
...
def draw_ui(stdscr, runner: ProgramRunner) -> None:
    ...
    # Draw actor usage
    actor_percentage = (runner.actor_usage / runner.actors_available * 100) if runner.actors_available > 0 else 0
    actor_str = f" Actor Usage: {runner.actor_usage:.1f}/{runner.actors_available} ({actor_percentage:.0f}%) "
    safe_addstr(2, (width - len(actor_str)) // 2, actor_str, curses.color_pair(1) if actor_percentage > 80 else curses.A_NORMAL)
...
def handle_input(stdscr, runner: ProgramRunner) -> bool:
    ...
    if key == 'q':
        ...
        if selected_step.status == StepStatus.PENDING and selected_step.has_manual_trigger():
            # Trigger the step to start
            runner.command_queue.put(f"trigger:{selected_step.manual_trigger_name}:{selected_step_id}")
        elif selected_step.status == StepStatus.RUNNING and (selected_step.duration_type == "variable" or selected_step.duration_type == "indefinite"):
            ...
            if selected_step.duration_type == "variable":
                ...
                min_progress = (selected_step.min_seconds / selected_step.default_seconds) * 100
        ...
        for i, trigger in enumerate(available_triggers):
            if i < menu_height - 4:
                if trigger["type"] == "program":
                    ...
                    menu_win.addstr(i + 2, 2, f"{i+1}. {trigger['name']} (Track: {trigger['track_id']})")
                    ...
                    if selection and selection.isdigit():
                        ...
                        if 0 <= idx < len(available_triggers):
                            ...
                            if trigger["id"] == "start_program":
                                ...
                                trigger_type, trigger_name, step_id = trigger["id"].split(":", 2)
                                runner.command_queue.put(f"trigger:{trigger_name}:{step_id}")
                    break
                elif ch.isdigit():
                    selection += ch
            except:
...
def run_program(program_file: str, schema_file: str = 'program_schema.json', 
                time_scale: float = 1.0, validate: bool = True, auto_start: bool = False, 
    ...
    if environment:
        ...
        if default_env:
            ...
            print(f"Using default environment '{default_env}' for type '{environment_type}'")
        else:
...
def main():
    ...
    parser.add_argument('--schema', default='program_schema.json', help='Path to the schema file (default: program_schema.json)')
    parser.add_argument('--time-scale', type=float, default=1.0, help='Time scale factor (default: 1.0)')
    parser.add_argument('--validate', action='store_true', help='Validate the program before running')
    parser.add_argument('--auto-start', action='store_true', help='Automatically start the program')
    
    args = parser.parse_args()
    
    run_program(args.program_file, args.schema, args.time_scale, args.validate, args.auto_start)
