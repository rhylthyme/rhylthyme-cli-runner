            ...
            for resource_id, count_change in self.usage_profile[current_time].items():
                ...
                if current_usage[resource_id] > 0:
                    if resource_id not in result:
                        result[resource_id] = []
                    result[resource_id].append((current_time, next_time, current_usage[resource_id]))
        
        return result
    
    def find_bottlenecks(self, threshold: int = 2) -> List[Tuple[str, float, float, int]]:
        """
...
class Step:
    ...
    def __init__(self, step_data: dict, track_id: str):
        ...
        self.priority = step_data.get('priority', 100)  # Default priority is 100 (lower is higher priority)
        ...
        if isinstance(self.duration_data, dict):
            ...
            if self.duration_type == 'fixed':
                ...
                self.optimal_duration = step_data.get('duration', 0)  # Fallback to simple duration
        else:
...
class ProgramPlanner:
    ...
    def __init__(self, program: dict, verbose: bool = False, environment: dict = None):
        ...
        if environment:
            ...
            if prog_type and env_type and prog_type != env_type:
                raise ValueError(f"Environment type '{env_type}' does not match program environmentType '{prog_type}'")
            ...
            for track_id, step_id, step in steps_to_start:
                ...
                for resource in step.resources:
                    self.resource_usage.add_usage(
                        start_time,
                        start_time + step.calculate_duration(),
                        resource
                    )
                ...
                for resource in step.resources:
                    self.min_resource_usage.add_usage(
                        start_time,
                        start_time + step.get_min_duration(),
                        resource
                    )
                    self.max_resource_usage.add_usage(
                        start_time,
                        start_time + step.get_max_duration(),
                        resource
                    )
    ...
    def optimize_schedule(self) -> dict:
        ...
        if self.resource_constraints or self.equipment_constraints:
            ...
            if self.resource_constraints:
                task_limits = {rc['task']: rc.get('maxConcurrent', float('inf')) for rc in self.resource_constraints}
            
            # Map equipment constraints
            equipment_limits = {}
            if self.equipment_constraints:
                equipment_limits = {eq['id']: eq.get('maxConcurrent', float('inf')) for eq in self.equipment_constraints}
            
            # Check task-based bottlenecks
            for resource_id, usage_periods in self.resource_usage.calculate_usage_profile().items():
        ...
        if self.verbose and combined_bottlenecks:
            ...
            for resource_id, start_time, end_time, count in combined_bottlenecks:
                print(f"  Resource '{resource_id}': {count} concurrent uses from {start_time} to {end_time}")
                ...
                if start_trigger.get('type') in ['afterStep', 'afterStepWithBuffer']:
                    ...
                    if referenced_step_id not in step_id_to_index:
                        ...
                        if i > 0:
                            ...
                            if self.verbose:
                                print(f"Fixed step '{step.get('name', step.get('stepId'))}' to start after '{prev_step.get('name', prev_step.get('stepId'))}'")
                        else:
                            # First step should start with program
                            step['startTrigger'] = {'type': 'programStart'}
                            if self.verbose:
                                print(f"Fixed step '{step.get('name', step.get('stepId'))}' to start with program")
                    else:
                        ...
                        if ref_index >= i:
                            ...
                            # Update indices
                            step_id_to_index = {step.get('stepId'): j for j, step in enumerate(steps)}
                            if self.verbose:
                                print(f"Moved step '{ref_step.get('name', ref_step.get('stepId'))}' before '{step.get('name', step.get('stepId'))}'")
                ...
                if isinstance(duration, dict) and duration.get('type') == 'variable':
                    ...
                    if 'optimalSeconds' in duration:
                        ...
                        if min_seconds <= optimal <= max_seconds:
                            ...
                            if self.verbose:
                                print(f"Adjusted default duration for step '{step.get('id', '')}' to optimal value: {optimal}")
                        
    def _can_start(self, step: Step, completed: Set[Tuple[str, str]], 
                  start_times: Dict[str, Dict[str, float]]) -> bool:
        """
    ...
    def _calculate_start_time(self, step: Step, 
                             start_times: Dict[str, Dict[str, float]]) -> float:
        """
    ...
    def _stagger_track_starts(self, program: dict, 
                             bottlenecks: List[Tuple[str, float, float, int]]):
        """
        ...
        for i, track in enumerate(tracks):
            ...
            if track_steps:
                avg_priority = sum(step.get('priority', 100) for step in track_steps) / len(track_steps)
        ...
        for resource, track_indices in resource_to_tracks.items():
            ...
            for i, track_index in enumerate(track_indices):
                ...
                if self.verbose:
                    print(f"Staggered track '{track.get('id', '')}' start time to {track['startTime']} (priority: {track_priorities[track_index]:.1f})")
    
    def _stagger_step_starts(self, program: dict, 
                            bottlenecks: List[Tuple[str, float, float, int]]):
        """
        ...
        for track in program.get('tracks', []):
            ...
            for step_index in bottleneck_step_indices:
                ...
                bottleneck_step_indices = [i + 1 if i >= step_index else i for i in bottleneck_step_indices]
                
                if self.verbose:
                    step_priority = track.get('steps', [])[step_index + 1].get('priority', 100)  # +1 because we inserted a step
                    print(f"Added padding step before step {step_index} in track '{track.get('id', '')}' (priority: {step_priority})")
    ...
    def _preserve_required_fields(self, program: dict):
        ...
        required_fields = [
            'programId', 'name', 'description', 'version', 'environment', 
            'environmentType', 'actors', 'duration', 'startTrigger', 
            'resourceConstraints', 'trackTemplates', 'metadata'
        ]
        ...
        if ('environment' in program or 'environmentType' in program) and 'resourceConstraints' not in self.program:
...
def plan_program(input_file: str, output_file: str, verbose: bool = False, environment_file: str = None) -> bool:
    """
...
if __name__ == "__main__":
    # Example usage
    plan_program("examples/breakfast_schedule.json", "examples/optimized_breakfast_schedule.json", verbose=True) 