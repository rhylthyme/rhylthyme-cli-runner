...
def perform_additional_validations(program: Dict[str, Any], strict: bool = False) -> List[str]:
    """
    ...
    for track in program.get("tracks", []):
        for step in track.get("steps", []):
            ...
            if start_trigger.get("type") in ["afterStep", "afterStepWithBuffer"] and "stepId" in start_trigger:
    ...
    if strict or ("actors" not in program and "environment" not in program):
        for task_type in task_types_in_steps:
            if task_type not in task_types_in_constraints:
                errors.append(f"Task '{task_type}' is used in steps but not defined in resourceConstraints")
    ...
    for ref_id in referenced_template_ids:
        if ref_id not in template_ids:
            errors.append(f"Referenced template ID '{ref_id}' does not exist in trackTemplates")
...
def calculate_step_start_time(step: Dict[str, Any], track_steps: List[Dict[str, Any]], program: Dict[str, Any]) -> int:
    """
    ...
    if trigger_type == "programStart":
        ...
        referenced_start_time = calculate_step_start_time(referenced_step, referenced_track_steps, program)
        referenced_duration = parse_duration_to_seconds(referenced_step.get("duration", "0s"))
...
def validate_program_file_structured(program_file: str, schema_file: str = 'program_schema.json', verbose: bool = False, strict: bool = False) -> dict:
    """
    ...
    summary = {
        ...
        'totalSteps': sum(len(track.get('steps', [])) for track in program.get('tracks', [])),
    }
    return {
...
def validate_program_file(program_file: str, schema_file: str = 'program_schema.json', verbose: bool = False, json_output: bool = False, strict: bool = False) -> bool:
    """
    ...
    """
    result = validate_program_file_structured(program_file, schema_file, verbose, strict)
    if json_output:
...
def main():
    parser = argparse.ArgumentParser(description='Validate program files (JSON or YAML) against the schema')
    parser.add_argument('program_files', nargs='+', help='Program files (JSON or YAML) to validate')
    ...
    parser.add_argument('--json', '-j', action='store_true', help='Print machine-readable JSON result')
    parser.add_argument('--strict', '-s', action='store_true', help='Enforce all tasks must be defined in resourceConstraints')
    ...
    for program_file in args.program_files:
        file_valid = validate_program_file(program_file, args.schema, args.verbose, args.json, args.strict)
        if not file_valid:
            all_valid = False
