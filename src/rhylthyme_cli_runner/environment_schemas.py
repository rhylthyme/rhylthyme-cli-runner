...
BASE_ENVIRONMENT_SCHEMA = {
    ...
    "properties": {
        ...
        "type": {
            ...
            "enum": [
                "kitchen", "laboratory", "bakery", "airport", "restaurant", 
                "home", "commercial-kitchen", "lab", "research", "biotech", 
                "pharma", "medical", "artisan", "pastry", "aviation", 
                "runway", "terminal", "manufacturing", "warehouse", "office", 
                "hospital", "school", "retail", "farm", "datacenter", 
                "factory", "workshop", "garage", "gym", "studio", "theater", 
                "library", "garden", "greenhouse", "clinic", "spa", "hotel"
            ]
        },
        "actors": {
            "type": "integer",
...
ENVIRONMENT_TYPE_TASKS = {
    "kitchen": {
        ...
        "common_tasks": [
            "stove-burner", "oven", "microwave", "toaster", "prep-work", 
            "plating", "cleanup", "mixer", "refrigerator", "dishwasher",
            "grill", "fryer", "steamer", "food-processor", "walk-in-cooler",
            "freezer", "expediting", "service-window", "prep-station",
            "cold-prep", "grill-station", "pastry-oven", "dishwashing"
        ],
        "actor_types": [
            "home-cook", "chef", "cook", "prep-cook", "head-chef", 
            "sous-chef", "line-cook", "dishwasher", "chef-de-partie", 
            "commis-chef", "kitchen-porter", "pastry-chef", "expediter"
        ]
    },
    "laboratory": {
        "required_tasks": ["bench-space", "cleanup"],
        "common_tasks": [
            "fume-hood", "biosafety-cabinet", "incubator", "centrifuge",
            "microscope", "spectrophotometer", "pcr-machine", "hplc",
            "mass-spectrometer", "flow-cytometer", "bench-space",
            "prep-work", "autoclave", "freezer-80", "liquid-nitrogen",
            "data-analysis", "cleanup"
        ],
        "actor_types": [
            "principal-investigator", "lab-manager", "senior-scientist", 
            "postdoc", "grad-student", "lab-technician", "undergrad"
        ]
    },
    "bakery": {
        "required_tasks": ["oven", "mixer", "work-bench"],
        "common_tasks": [
            "oven", "convection-oven", "proofer", "mixer", "dough-sheeter",
            "work-bench", "prep-work", "decorating-station", "cooling-rack",
            "packaging", "cleanup", "refrigeration", "freezer", "laminator"
        ],
    ...
    "airport": {
        ...
        "common_tasks": [
            "runway", "gate", "baggage-handling", "check-in-counter",
            "security-checkpoint", "air-traffic-control", "ground-crew",
            "fueling-station", "maintenance-hangar", "cargo-terminal",
            "customs", "immigration"
        ],
        "actor_types": [
            "pilot", "air-traffic-controller", "ground-crew", "baggage-handler",
            "security-officer", "check-in-agent", "maintenance-technician"
        ]
    }
}
...
class EnvironmentValidator:
    ...
    def _validate_type_specific(self, environment_data: Dict[str, Any], env_type: str) -> List[str]:
        """Validate type-specific requirements."""
        errors = []
        ...
        if missing_required:
            errors.append(f"Missing required tasks for {env_type}: {', '.join(missing_required)}")
        ...
        if unexpected_tasks:
            ...
            errors.append(f"Warning: Uncommon tasks for {env_type}: {', '.join(unexpected_tasks)}")
        
        return errors
    
    def _validate_actor_types(self, environment_data: Dict[str, Any], env_type: str) -> List[str]:
        ...
        if 'actorTypes' in environment_data:
            ...
            if unexpected_actors:
                errors.append(f"Warning: Uncommon actor types for {env_type}: {', '.join(unexpected_actors)}")
        
        return errors
    
    def _validate_resource_constraints(self, environment_data: Dict[str, Any]) -> List[str]:
        ...
        for constraint in resource_constraints:
            ...
            for actor_type in qualified_types:
                if actor_type not in all_actor_type_ids:
                    errors.append(f"Task '{constraint['task']}' references unknown actor type: {actor_type}")
        
        # Validate actors vs actorsRequired totals don't exceed capacity
        total_actors = 0
        if 'actorTypes' in environment_data:
            total_actors = sum(actor_info['count'] for actor_info in actor_types.values())
        ...
        if max_required > total_actors:
            errors.append(f"Resource constraints require up to {max_required} actors but only {total_actors} available")
...
def validate_all_environments(environments_dir: str = "environments") -> Dict[str, List[str]]:
    """
...
if __name__ == "__main__":
    ...
    for env_type in sorted(validator.list_supported_types()):
        ...
        print(f"  {env_type:20} - Required: {', '.join(required) if required else 'None'}")
